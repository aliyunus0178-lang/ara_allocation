...
export const MyAraProfile: React.FC<MyAraProfileProps> = ({
  ...
  // Calculate potential overlaps from session timings (SRS timing constraints)
  const conflictsBySessionId = useMemo(() => {
    const map = new Map<string, { conflictingShift: typeof myShifts[0]; overlapMessage: string }[]>();

    for (let i = 0; i < myShifts.length; i++) {
      const shiftA = myShifts[i];
      const sessA = shiftA.session;
      if (!sessA) continue;
      const startA = parseTimeToMinutes(sessA.start_time);
      const endA = parseTimeToMinutes(sessA.end_time);

      for (let j = 0; j < myShifts.length; j++) {
        if (i === j) continue;
        const shiftB = myShifts[j];
        const sessB = shiftB.session;
        if (!sessB) continue;

        // Must be same day of the week to overlap
        if (sessA.day_of_week !== sessB.day_of_week) continue;

        const startB = parseTimeToMinutes(sessB.start_time);
        const endB = parseTimeToMinutes(sessB.end_time);

        // Standard interval overlap condition: max(startA, startB) < min(endA, endB)
        if (Math.max(startA, startB) < Math.min(endA, endB)) {
          const existing = map.get(sessA.id) || [];
          const courseBName = shiftB.course?.course_code || sessB.course_id;
          existing.push({
            conflictingShift: shiftB,
            overlapMessage: `${courseBName} (${sessB.start_time} - ${sessB.end_time} in ${shiftB.room?.room_code || 'Lab'})`,
          });
          map.set(sessA.id, existing);
        }
      }
    }
    return map;
  ...
  const totalConflictsCount = conflictsBySessionId.size;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Schedule Timing Overlap Warning Banner (SRS §7 & §16 Hard Constraint Violation Alert) */}
      {totalConflictsCount > 0 && (
        <div className="bg-amber-50 dark:bg-amber-950/40 border-2 border-amber-500/80 rounded-xl p-4 sm:p-5 shadow-md flex items-start gap-3.5 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="p-2 bg-amber-500 text-slate-950 rounded-lg shrink-0 mt-0.5 shadow-xs">
            <AlertTriangle className="w-5 h-5" />
          </div>
          <div className="flex-1">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h4 className="text-sm font-bold text-amber-900 dark:text-amber-200 font-serif flex items-center gap-2">
                <span>Timetable Schedule Conflict Warning</span>
                <span className="bg-amber-200 dark:bg-amber-900/60 text-amber-950 dark:text-amber-300 font-mono text-[10px] font-bold px-2 py-0.5 rounded border border-amber-400">
                  {totalConflictsCount} Overlapping Slot{totalConflictsCount > 1 ? 's' : ''} Detected
                </span>
              </h4>
              <span className="text-[11px] font-semibold text-amber-700 dark:text-amber-400">
                SRS §7 / §16 Time Constraint
              </span>
            </div>
            <p className="text-xs text-amber-800 dark:text-amber-300 mt-1">
              You are assigned/registered for two or more overlapping laboratory sessions that take place simultaneously on the same teaching day. Please review the highlighted shifts below and request an assignment reallocation or substitute.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs uppercase tracking-wider font-bold text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-950/50 px-2 py-0.5 rounded border border-sky-200 dark:border-sky-800 flex items-center gap-1">
                <FileCheck className="w-3.5 h-3.5 text-sky-600 dark:text-sky-400" />
                Schedule of Laboratory Shifts
              </span>
            </div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white mt-1 font-serif">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="text-slate-500 dark:text-slate-400 font-medium">Filter Day:</span>
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-xs font-semibold transition-all ${
                    selectedDayFilter === day
                      ? 'bg-white dark:bg-slate-700 text-[#002147] dark:text-amber-300 shadow-xs font-bold'
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {filteredShifts.map(({ assignment, session, course, room, block }) => {
                  ...
                  const conflicts = session ? conflictsBySessionId.get(session.id) : undefined;
                  ...
                  return (
                    <tr 
                      ...
                      className={`transition-colors ${
                        hasConflict 
                          ? 'bg-amber-50/60 dark:bg-amber-950/20 border-l-4 border-l-amber-500 hover:bg-amber-50 dark:hover:bg-amber-950/30' 
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 font-mono font-bold text-slate-900 dark:text-white">
                        <div className="flex items-center gap-1.5">
                          <span className={`w-2 h-2 rounded-full ${hasConflict ? 'bg-amber-500 animate-ping' : 'bg-amber-500'}`}></span>
                          <span>{session?.day_of_week}</span>
                          {hasConflict && (
                            <span 
                              className="text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/60 p-1 rounded-full border border-amber-300 dark:border-amber-700/80 ml-1 inline-flex items-center justify-center cursor-help"
                              title={`Timing Overlap Warning: Conflicting with ${conflicts.map(c => c.overlapMessage).join('; ')}`}
                            >
                              <AlertTriangle className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
                            </span>
                          )}
                        </div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 font-normal pl-3.5">
                          {session?.start_time} - {session?.end_time}
                        </div>
                        {hasConflict && (
                          <div className="mt-1 pl-3.5">
                            <span className="inline-block text-[10px] font-sans font-bold text-amber-800 dark:text-amber-300 bg-amber-100 dark:bg-amber-950/80 border border-amber-300 dark:border-amber-800/80 px-1.5 py-0.5 rounded">
                              ⚠️ Overlaps: {conflicts[0].overlapMessage}
                            </span>
                          </div>
                        )}
                      </td>
                      ...
                      {/* Course */}
                      <td className="py-3.5 px-4">
                        <div className="font-bold text-[#002147] dark:text-sky-300 flex items-center gap-1.5">
                          <span>{course?.course_code}</span>
                          <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-1.5 py-0.2 rounded font-mono">
                            {course?.program || 'CSE'}
                          </span>
                        </div>
                        <div className="text-[11px] text-slate-600 dark:text-slate-400 max-w-xs truncate" title={course?.course_name}>
                          {course?.course_name}
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 px-4 font-mono">
                        <span className="bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 px-2 py-0.5 rounded font-semibold border border-slate-200 dark:border-slate-700">
                      ...
                      <td className="py-3.5 px-4">
                        {isKeyHolder ? (
                          <span className="bg-amber-50 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 border border-amber-300 dark:border-amber-700 px-2 py-0.5 rounded font-bold text-[10px] flex items-center gap-1 w-fit">
                            <KeyRound className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                            Room Key-Holder
                          </span>
                        ) : (
                          <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded font-medium text-[10px] border border-slate-200 dark:border-slate-700">
                        ...
                        {assignment.status !== 'Confirmed' ? (
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <button
                              ...
                              className="px-2 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-rose-100 dark:hover:bg-rose-950 text-rose-700 dark:text-rose-400 rounded text-[11px] border border-slate-300 dark:border-slate-700 hover:border-rose-300"
                          ...
                          <span className="text-emerald-700 dark:text-emerald-400 font-medium text-[11px] flex items-center justify-end gap-1">
