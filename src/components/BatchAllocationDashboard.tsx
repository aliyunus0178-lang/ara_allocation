      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5 transition-colors">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            {/* High-Precedence Capstone Auto-Fill Utility */}
            {onHighPrecedenceAutoFill && (
              <button
                onClick={onHighPrecedenceAutoFill}
                className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white bg-[#002147] hover:bg-[#001733] rounded-lg shadow-sm border border-amber-500/40 transition-all cursor-pointer"
                title="Scan for unassigned Capstone & Final Year Project sessions and auto-fill with highest-rated SARAs"
              >
                <Award className="w-3.5 h-3.5 text-amber-400" />
                <span>Capstone Auto-Fill</span>
              </button>
            )}
                              ...
                              {sessionAssignments.length === 0 ? (
                                ...
                                <div className="space-y-1">
                                  {sessionAssignments.map((asgn) => {
                                    const ara = aras.find((a) => a.id === asgn.ara_id);
                                    const is510 = room?.block_id === 'block-510' || room?.room_code?.startsWith('510-');
                                    const roomResp = roomResponsibilities.find((rr) => rr.room_id === room?.id && rr.status === 'Active');
                                    const isVerifiedKeyHolder = is510 && roomResp && roomResp.ara_id === asgn.ara_id;
                                    
                                    // Check if ARA is identified as a room holder in 510-block
                                    const is510Holder = roomResponsibilities.some((rr) => {
                                      if (rr.ara_id !== ara?.id || rr.status !== 'Active') return false;
                                      const r = rooms.find((rm) => rm.id === rr.room_id);
                                      return r?.block_id === 'block-510' || r?.room_code?.startsWith('510-');
                                    });

                                    return (
                                      <div key={asgn.id} className="flex flex-wrap items-center gap-1.5 text-xs">
                                        ...
                                        {is510Holder && (
                                          <span className="inline-flex items-center gap-0.5 bg-amber-100 text-amber-900 dark:bg-amber-950/80 dark:text-amber-200 border border-amber-300 dark:border-amber-700 px-1.5 py-0.2 rounded text-[9px] font-bold shadow-2xs">
                                            <Award className="w-2.5 h-2.5 text-amber-600 dark:text-amber-400 shrink-0" />
                                            Certified SARA
                                          </span>
                                        )}
                                        {isVerifiedKeyHolder && (
                                          <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-900 dark:bg-emerald-950/80 dark:text-emerald-300 dark:border-emerald-700 px-1.5 py-0.2 rounded text-[10px] font-mono font-bold border border-emerald-300 shadow-2xs">
                    ...
                    <div className="text-xs">
                      ...
                      {sessionAssignments.length === 0 ? (
                        ...
                        sessionAssignments.map((asgn) => {
                          ...
                          return (
                            <div key={asgn.id} className="flex flex-wrap items-center justify-between gap-1 p-1.5 bg-slate-100/80 dark:bg-slate-800 rounded mb-1 font-mono text-xs">
                              <div className="flex flex-wrap items-center gap-1.5">
                                <span className="font-bold text-slate-800 dark:text-slate-200">{ara?.ara_code} ({ara?.full_name.split(' ')[0]})</span>
                                {is510Holder && (
                                  <span className="inline-flex items-center gap-0.5 bg-amber-100 text-amber-900 dark:bg-amber-950/80 dark:text-amber-200 border border-amber-300 dark:border-amber-700 px-1 py-0.2 rounded text-[9px] font-bold">
                                    <Award className="w-2.5 h-2.5 text-amber-600 dark:text-amber-400 shrink-0" />
                                    Certified SARA
                                  </span>
                                )}
                                {isVerifiedKeyHolder && (
                                  <span className="inline-flex items-center gap-0.5 bg-emerald-100 text-emerald-900 dark:bg-emerald-950/80 dark:text-emerald-300 dark:border-emerald-700 px-1 py-0.2 rounded text-[9px] font-bold border border-emerald-300">
